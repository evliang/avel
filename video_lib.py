...
import shutil
import subprocess
from shared_lib import call_external, get_duration
...
def trim_video(inp, start_time, end_time, outp):
    ...
    subprocess.call(['ffmpeg', '-y', 
                    '-i', inp, '-ss', start_time, '-to', end_time, outp])

def create_image_video(image, seconds, output_filename):
    """creates video from single image. seconds determines length of video"""
    subprocess.call(['ffmpeg', '-hide_banner', '-loglevel', 'panic', '-loop', '1', '-i', image, '-c:v', 'libx264', '-t', str(seconds), '-pix_fmt', 'yuv420p', '-vf', 'pad=ceil(iw/2)*2:ceil(ih/2)*2', output_filename])
...
def create_image_slideshow_fade(images, seconds, output_filename):
    ...
    subprocess.call(['ffmpeg', '-hide_banner', '-loglevel', 'panic', '-y'] + part1 + ['-filter_complex'] + [part2 + part3] + ['-map', '[v]', output_filename])
...
def concat_videos(files, output_filename, debugging=False):
    ...
    if (len(files) < 400):
        for (idx, f) in enumerate(files):
            subprocess.call(['ffmpeg', '-y', '-i', f, '-c', 'copy', '-bsf:v', 'h264_mp4toannexb', '-f', 'mpegts', intermediate_file(idx)])
        ...
        subprocess.call(['ffmpeg',
                ...
                '-f', 'concat', '-safe', '0',
                '-i', 'templist.txt', '-codec', 'copy', output_filename])
