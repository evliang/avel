...
def combine_audio(audio_list, output_path, transition_time=13, debugging=False):
    ...
    if len(audio_list) > 2:
        print(audio_list)
        call_ffmpeg(f'ffmpeg -i {audio_list[0]} -i {audio_list[1]} -vn -filter_complex acrossfade=d={transition_time}:c1=tri:c2=squ {temp1}')
        for i in range(2, len(audio_list) - 1):
            call_ffmpeg(f'ffmpeg -i {temp_file(i-1)} -i {audio_list[i]} -vn -filter_complex acrossfade=d={transition_time}:c1=tri:c2=squ {temp_file(i)}')
        # final call to convert to mp3
        call_ffmpeg(f'ffmpeg -i {temp_file(len(audio_list) - 2)} -i {audio_list[-1]} -vn -filter_complex acrossfade=d={transition_time}:c1=tri:c2=squ {output_path}')
    elif len(audio_list) == 2:
        call_ffmpeg(f'ffmpeg -i {audio_list[0]} -i {audio_list[1]} -vn -filter_complex acrossfade=d={transition_time}:c1=tri:c2=squ {output_path}')
...
def extract_audio(video_file, time1, time2, output_file):
    ...
    call_ffmpeg(f'ffmpeg -i {video_file} -ss {time1} -to {time2} -c:a libmp3lame {output_file}')
...
def merge_audio(audio_file1, audio_file2, output_file, vol1=1.0, vol2=1.0):
    ...
    call_ffmpeg(f'ffmpeg -i {audio_file1} -i {audio_file2} -filter_complex [0:0]volume={vol1}[a];[1:0]volume={vol2}[b];[a][b]amix=inputs=2:duration=longest -c:a libmp3lame {output_file}')